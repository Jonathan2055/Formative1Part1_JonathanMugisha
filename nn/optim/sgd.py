    ...
    def __init__(
        ...
        Sets:
            self.parameters (list[tuple[np.ndarray, np.ndarray]]):
                the (param, grad) pairs to update, stored as
                given -- same array references as each module's
                own self.W/self.dW etc., so updates and resets
                here are visible to those modules too.
